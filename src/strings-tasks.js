...
function sumOfCodes(str) {
  if (str !== null && str !== undefined)
    return str.split('').reduce((sum, curr) => sum + curr.charCodeAt(0), 0);
  return 0;
}
...
function countVowels(str) {
  const sort = str.match(/[aeiouyAEIOUY]/gi);
...
function isPalindrome(str) {
  const completeStr = str.toLowerCase().replace(/[^a-z]/g, '');
  const reverseStr = completeStr.split('').reverse().join('');
  return completeStr === reverseStr;
}
...
function extractNameFromTemplate(value) {
  const match = value.match(/Hello, (\w+) (\w+)!/);
...
// const str = 'Why did the chicken cross the road?';
// const result = [];
// str.split('').forEach((i) => {
//   if (i.match(/[a-zA-Z]/)) {
//     if (i <= 'M' || (i >= 'a' && i <= 'm')) {
//       result.push(String.fromCharCode(i.charCodeAt(0) + 13));
//     } else {
//       result.push(String.fromCharCode(i.charCodeAt(0) - 13));
//     }
//   } else {
...
function getCardId(value) {
  ...
  const suits = ['♣', '♦', '♥', '♠'];
  const cardIndex = cards.indexOf(value.slice(0, -1));
  const suitIndex = suits.indexOf(value[value.length - 1]);
  return suitIndex * cards.length + cardIndex;
}
